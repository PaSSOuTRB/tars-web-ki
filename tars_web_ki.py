import streamlit as st import openai import requests import tempfile import os
...
def speak_with_elevenlabs(text): url = f"https://api.elevenlabs.io/v1/text-to-speech/Arnold" headers = { "xi-api-key": elevenlabs_api_key, "Content-Type": "application/json" } data = { "text": text, "model_id": "eleven_multilingual_v2", "voice_settings": { "stability": 0.6, "similarity_boost": 0.7 } } response = requests.post(url, headers=headers, json=data) if response.status_code == 200: with tempfile.NamedTemporaryFile(delete=False, suffix=".mp3") as fp: fp.write(response.content) st.audio(fp.name, format="audio/mp3") else: st.error("Fehler bei der Sprachausgabe von ElevenLabs")
